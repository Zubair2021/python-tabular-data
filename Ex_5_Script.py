...
"""
Regression and plot of two variables stored in columns of a tabular data file.
"""
...
# Defining a function for automatically composing a plot file name

def compose_plot_file_name(x_axis_label, y_axis_label, category_label = None):
    ...
    plot_path = "{0}-v-{1}{2}.png".format(x_axis_label, y_axis_label,
            category_str)
    return plot_path

# Defining a Function for Regression and Scatter Plot
...
def regress_and_scatter(dataframe, x_column_name, y_column_name,
    ...
    for category, df in grouped_dataframes:
        ...
        plt.scatter(x, y, label = category, marker = '1', color = 'C' + str(color_index), alpha=0.5, cmap='viridis')
...
def main_cli():
    ...
    # Use arg parser to parse the command-line args
